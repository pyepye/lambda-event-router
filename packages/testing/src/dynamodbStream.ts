...
export interface DynamoDBStreamHandlerEvent {
...
export function createDynamoDBStreamEvent(records: DynamoDBRecord[] = [createDynamoDBRecord()]): DynamoDBStreamEvent {
  return { Records: records };
}

export interface CreateDynamoDBStreamHandlerEventOptions {
  records?: DynamoDBRecord[];
  context?: Partial<Context>;
}

export function createDynamoDBStreamHandlerEvent(
  options: CreateDynamoDBStreamHandlerEventOptions = {},
): DynamoDBStreamHandlerEvent {
  const event = createDynamoDBStreamEvent(options.records);
