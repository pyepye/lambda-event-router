...
export async function createItem({ newImage, keys }: DynamoDBInsertRequest<Keys, Item>): Promise<DynamoDBResponse> {
  const { pk, sk } = keys;
...
export async function updateItem({
  ...
  oldImage,
  keys,
}: DynamoDBRequest<Keys, Item, Item>): Promise<DynamoDBResponse> {
  const { pk, sk } = keys;
...
export async function modifyItem({
  ...
  oldImage,
  keys,
}: DynamoDBModifyRequest<Keys, Item, Item>): Promise<DynamoDBResponse> {
  const { pk, sk } = keys;
  console.log(
...
export async function removeItem({ oldImage, keys }: DynamoDBRemoveRequest<Keys, Item>): Promise<DynamoDBResponse> {
  const { pk, sk } = keys;
